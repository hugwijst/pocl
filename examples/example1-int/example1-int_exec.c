...
exec_dot_product_kernel(const char *program_source,
                        int n, cl_int4 *srcA, cl_int4 *srcB, cl_int *dst)
{
  cl_context  context;
  ...
  for (i = 0; i < n; ++i) {
    poclu_bswap_cl_int_array(devices[0], (cl_int*)&srcA[i], 4);
    poclu_bswap_cl_int_array(devices[0], (cl_int*)&srcB[i], 4);
  }
